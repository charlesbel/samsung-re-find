...
- **Shared Master State Contract (v1):** Introduced neutral `master-state-v1` schema (`schemas/master-state-v1.schema.json`) stored at `samsung-account/master.json` for seamless shared authentication with `samsung-health-cloud`.
- **Non-Destructive State Migration:** Added `samsung-find migrate-master` command to upgrade v0.1 state files to `master-state-v1` without re-login.
- **Model Context Protocol (MCP) Server:** Added `samsung-find-mcp` stdio server exposing typed tools for AI agents, with physical side effects gated behind `--allow-effects`.
...
- **Portable Skills:** Moved and standardized skills under `.skills/samsung-find/` and `.skills/samsung-account-auth/`.
- **Package Typing:** Added `py.typed` PEP 561 marker.

### Changed
- **Renamed Distribution:** Package renamed from `samsung-find-agent` to `samsung-find` (`samsung_find` import and `samsung-find` command preserved).
